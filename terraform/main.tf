...
resource "azurerm_container_app" "emailproc" {
  name                         = "aca-emailproc-${local.func_name}"
  ...
  template {
    container {
      name   = "agent"
      image  = "ghcr.io/${var.gh_repo}/email-processing-agent:latest"
      cpu    = 0.25
      memory = "0.5Gi"
...
resource "azurerm_container_app" "bot" {
  ...
  template {
    container {
      name   = "bot"
      image  = "ghcr.io/${var.gh_repo}/copilot-agent:latest"
      cpu    = 0.5
      memory = "1Gi"
